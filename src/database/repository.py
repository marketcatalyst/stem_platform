...
class ProjectPersistenceRepository:
    ...
    def get_or_create_site_by_name(
        ...
        with Session(self.engine) as session:
            try:
                # Using parameter mapping that the engine handles internally
                stmt = text("""
                    SELECT site_id FROM client_sites 
                    WHERE tenant_id = :tid AND client_name = :name LIMIT 1;
                """)
                res = session.execute(
                    stmt, {"tid": tenant_uuid, "name": clean_name}
                ).fetchone()
                ...
                session.execute(
                    insert_stmt,
                    {"id": new_site_id, "tid": tenant_uuid, "name": clean_name},
                )
                session.commit()
    ...
    def load_site_inventory_state(self, site_uuid_str: str) -> pd.DataFrame:
        ...
        with Session(self.engine) as session:
            try:
                result = session.execute(query, {"id": site_uuid}).fetchall()
                if not result:
                    return pd.DataFrame()
    ...
    def save_site_inventory_state(self, site_uuid_str: str, df: pd.DataFrame) -> dict:
        ...
        with Session(self.engine) as session:
            try:
                session.begin()
                session.execute(
                    text("DELETE FROM site_inventories WHERE site_id = :id;"),
                    {"id": site_uuid},
                )
                ...
                for _, row in df.iterrows():
                    session.execute(
                        text(
                            "INSERT INTO site_inventories (inventory_id, site_id, asset_tag) VALUES (:inv, :site, :tag);"
                        ),
                        {
                            "inv": uuid.uuid4(),
                            "site": site_uuid,
